...
import type { User, Workspace, Board, List, Card, ApiResponse, Comment, Checklist, Label } from '@/types';
...
export const authApi = {
    ...
    updateProfile: (data: { name?: string; avatar?: string }) => api.put<ApiResponse<User>>('/auth/profile', data),
};
...
export const workspaceApi = {
    ...
    create: (data: { name: string; description?: string }) => api.post<ApiResponse<Workspace>>('/workspaces', data),
    ...
    addMember: (id: string, email: string) => api.post<ApiResponse<Workspace>>(`/workspaces/${id}/members`, { email }),

    removeMember: (id: string, userId: string) => api.delete<ApiResponse<null>>(`/workspaces/${id}/members/${userId}`),
};

// Boards
export const boardApi = {
    getAll: (workspaceId: string) => api.get<ApiResponse<Board[]>>('/boards', { params: { workspaceId } }),
...
export const listApi = {
    getAll: (boardId: string) => api.get<ApiResponse<List[]>>('/lists', { params: { boardId } }),

    create: (data: { name: string; boardId: string }) => api.post<ApiResponse<List>>('/lists', data),

    update: (id: string, data: { name?: string }) => api.put<ApiResponse<List>>(`/lists/${id}`, data),

    delete: (id: string) => api.delete<ApiResponse<null>>(`/lists/${id}`),

    updatePosition: (id: string, position: number) => api.put<ApiResponse<List>>(`/lists/${id}/position`, { position }),
};

// Cards
export const cardApi = {
    getAll: (listId: string) => api.get<ApiResponse<Card[]>>('/cards', { params: { listId } }),
    ...
    addComment: (id: string, content: string) => api.post<ApiResponse<Comment>>(`/cards/${id}/comments`, { content }),

    addChecklist: (id: string, title: string) => api.post<ApiResponse<Checklist>>(`/cards/${id}/checklists`, { title }),
    ...
    deleteLabel: (id: string, labelId: string) => api.delete<ApiResponse<null>>(`/cards/${id}/labels/${labelId}`),
};
